	...
	stages {
		...
		stage ('Git Checkout'){
			steps {
				...
				git branch: 'dev', credentialsId: 'git-cred', url: 'https://github.com/mokadir/mkadir-blogapp.git'
			}
		}
			...
			steps {
				script {
					...
					withDockerRegistry(credentialsId: 'docer-cred') {
						sh "docker build -t mskr7/mkadir-blogapp:jar ."
					}
				}
			}
		}
     ...
     environment {  #environment variables for previous docker alternate stage
		...
		stage ('Docker Image Scan'){ 			
			steps {
				...
				sh "trivy image --scanners vuln --format table -o trivyscandocr.html mskr7/mkadir-blogapp:latest"
			}
		}  need lot of ram
*/		
		stage ('Docker Push'){
			steps {
				script {
					...
					withDockerRegistry(credentialsId: 'docer-cred') {
						sh "docker push mskr7/mkadir-blogapp:jar"
					}
				}
			}
		}
 		...
 		stage('Smoke Test') {
			steps { 
				...
				sh "docker run -d --name smokerun -p 8080:8080 mskr7/mkadir-blogapp:jar"
				sh "sleep 90"
